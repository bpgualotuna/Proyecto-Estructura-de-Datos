...
int ingresar_entero_32(const char *mensaje)
{
    char num[33]; // Buffer para almacenar hasta 32 caracteres más el terminador nulo
    char c;
    int i = 0;
    ...
    while (true)
    {
        ...
        { // Si es un número
            ...
            {                        // Verifica que no exceda el tamaño del buffer
                cout << c;           // Muestra el caracter
                num[i++] = c;        // Agrega al arreglo
                tiene_digito = true; // Marca que se ingresó al menos un dígito
            }
            else
            {
                cout << '\a'; // Beep si se excede el límite de 32 caracteres
            }
        }
        else if (c == 8 && i > 0)
        {                    // Si se presiona Backspace y hay algo que borrar
        ...
        { // Si se presiona Enter
            if (tiene_digito)
            { // Permitir Enter solo si se ingresó al menos un dígito
                break;
            }
            else
            {
        ...
        {
            cout << '\a'; // Beep para caracteres no válidos
        }
    }
...
int ingresar_entero_limitado(const char* mensaje, int min, int max) {
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (c >= '0' && c <= '9') { // Si es un número
                ...
                if (tiene_digito) { // Permitir Enter solo si se ingresó al menos un dígito
                    num[i] = '\0'; // Termina la cadena
                    valor = atoi(num); // Convierte la cadena a entero
                    if (valor >= min && valor <= max) { // Verifica el rango
                        cout << endl;
                        return valor; // Retorna si está en el rango
                    } else {
                        cout << "\nError: El valor debe estar entre " << min << " y " << max << ".\n";
                        break; // Vuelve a mostrar el mensaje para nuevo ingreso
                    }
                } else {
                    cout << '\a'; // Beep para indicar error
                }
...
float ingresar_float(const char *msj)
{
    ...
    { // Mientras no se presione Enter (ASCII 13)
        ...
        {
            if (i < 6) // Limitar a 6 caracteres en total
            {
                if (c == '.')
                {
            ...
            {
                printf("\a"); // Beep si se excede el límite de 6 caracteres
            }
        }
        else if (c == 8 && i > 0)
        {        // Si se presiona Backspace y hay algo que borrar
        ...
        {
            printf("\a"); // Beep para caracteres no válidos
        }
    }
    ...
    if (!decimal_point)
    {
        if (i <= 4) // Asegurar espacio para agregar .00
        {
            cad[i++] = '.';
            cad[i++] = '0';
            cad[i++] = '0';
...
float ingresar_saldo_inicial(const char *msj)
{
    while (true)
    {
        ...
        {
            if (i < 6) // Asegurar que no se exceda el límite al añadir un cero
            {
...
std::string ingresar_nombre_archivo(const char *mensaje)
{
    ...
    do
    {
        ...
        if (valido && (entrada.length() < 4 || entrada.substr(entrada.length() - 4) != ".txt"))
        {
            valido = false;
            std::cout << "El archivo debe tener extension .txt.\n";
        }
...
FechaHora ingresar_fecha_validada(const char *mensaje)
{
    ...
    while (!fechaValida)
    {
        ...
        while (anio < 1970 || anio > anioActual)
        {
...
FechaHora ingresar_fecha_solo_dia_validada(const char *mensaje)
{
    int dia, mes, anio;
    bool fechaValida = false;
    std::cout << mensaje;

    // Obtener el año actual para validar que la fecha sea razonable
    FechaHora ahora;
    ahora.actualizarHoraActual();
    int anioActual = ahora.obtenerAnio();
    ...
    while (!fechaValida)
    {
        ...
        FechaHora fechaCreacion(dia, mes, anio, 0, 0, 0);
        if (fechaCreacion.esFechaValida())
        {
            bool esBisiesto = fechaCreacion.esBisiesto();
            std::cout << "\nFecha valida: " << dia << "/" << mes << "/" << anio 
