...
void Menu::seleccionarSucursal() {
    ...
    while (true) {
        ...
        for (int i = 0; i < conteoOpciones; i++) {
            std::cout << (i == cursor ? "> " : "  ");
            std::cout << "Sucursal: " << opciones[i].nombre << " (Codigo: " << opciones[i].codigo << ")\n";
        }
...
void Menu::agendarCita() {
    if (!sucursalActual) {
        std::cout << "No se ha seleccionado ninguna sucursal.\n";
        std::cout << "Presione cualquier tecla para continuar...";
        _getch();
        return;
    }
    ...
    // Ingresar cédula
    std::string cedula = ingresar_cedula("Ingrese numero de cedula: ");

    // Ingresar fecha
    FechaHora fecha = ingresar_fecha_solo_dia_validada("Ingrese fecha de la cita:\n");
    FechaHora ahora;
    ahora.actualizarHoraActual();
    while (fecha < ahora.normalizarDia()) {
        std::cout << "No se pueden agendar citas en fechas pasadas. Ingrese una fecha válida.\n";
        fecha = ingresar_fecha_solo_dia_validada("Ingrese fecha de la cita:\n");
    }

    // Ingresar hora
    int hora = ingresar_entero_limitado("Ingrese hora (8-16): ", 8, 16);

    // Agendar cita
    if (sucursalActual->agendarCita(cedula, fecha, hora)) {
        char* nombreSucursal = sucursalActual->obtenerNombre();
        std::cout << "Cita agendada en " << nombreSucursal << ".\n";
...
void Menu::mostrarCitas() {
    ...
    char* nombreSucursal = sucursalActual->obtenerNombre();
    std::cout << "Mostrando citas para " << nombreSucursal << ":\n";
    delete[] nombreSucursal;
    sucursalActual->mostrarCitas();
    std::cout << "Presione cualquier tecla para continuar...";
    _getch();
}
        ...
        if (tecla == 224) {
            ...
            case 16: { // Visualización de árbol AVL
                ...
                const char* opcionesAVL[] = {
                    ...
                    "Visualizar AVL por edad de duenios de cuentas (Consola)",
                        ...
                        cuentas.recorrer([&](Cuenta* cta){
                            if (sel == cursor) {
                                ...
                                if (exito) {
                                    ...
                                    std::cout << "(El QR contiene solo el numero de cuenta y el nombre del titular)" << std::endl;
            ...
            case 20: { // Agendar Cita
                agendarCita();
                break;
            }
            case 21: { // Mostrar Citas
                mostrarCitas();
                break;
            }
            case 22: { // Salir
