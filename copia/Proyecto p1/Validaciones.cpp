        ...
        while ((c = _getch()) != 13) { // Mientras no se presione Enter (ASCII 13)
            if ((c >= '0' && c <= '9') || (c == '.' && !decimal_point)) {
                if (c == '.') {
                    ...
                    if (decimal_count > 2) continue; // Ignorar más de dos decimales
                }
                printf("%c", c);       // Mostrar el caracter ingresado
                cad[i++] = c;         // Guardarlo en el arreglo
            }
...
string ingresar_string(const char* mensaje) {
    while (true) {
        ...
        // Validar que el número de palabras sea exactamente 1
        if (numPalabras == 1) {
            return entrada; // Retorna como un objeto de tipo string
        } else {
            cout << "\nError: El nombre debe ser una sola palabra sin espacios (ejemplo: David).\n";
    ...
    while (true) {
        ...
        while (true) {
            ...
            if ((c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z') || c == ' ') {
                ...
                if (tiene_letra) { // Permitir Enter solo si se ingresó al menos una letra
                    break;
                } else {
                    cout << '\a'; // Beep para indicar error
                }
        ...
        // Validar que el número de palabras sea entre 1 y 4
        if (numPalabras >= 1 && numPalabras <= 4) {
            return entrada; // Retorna como un objeto de tipo string
        } else {
            cout << "\nError: El apellido debe contener de 1 a 4 palabras (ejemplo: Rodriguez, de la Cruz).\n";
            cout << "Intente de nuevo.\n";
            _getch(); // Esperar tecla antes de limpiar
        }
    }
