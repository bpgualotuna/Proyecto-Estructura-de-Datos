...
case 3: // Buscar por Fecha de Creación
{
    ...
    while (!entradaValida) {
        try {
            system("cls"); 
            FechaHora fechaBuscada = ingresar_fecha_solo_dia_validada("Ingrese Fecha de Creacion a buscar (DD MM AAAA HH MM SS):\n");
            FechaHora fechaNormalizada(fechaBuscada.obtenerDia(), fechaBuscada.obtenerMes(), fechaBuscada.obtenerAnio(), 0, 0, 0);
            ListaDobleCircular<Cuenta*> resultados = buscarPorFechaCreacion(fechaNormalizada);
            if (resultados.obtenerTamano() == 0) {
                char* fechaStr = fechaNormalizada.aCadena();
                std::cout << "No se encontraron cuentas con la fecha de creacion " << fechaStr << ".\n";
                delete[] fechaStr;
            } else {
                std::cout << "Cuentas encontradas para el " << fechaNormalizada.obtenerDia() << "/"
                          << fechaNormalizada.obtenerMes() << "/" << fechaNormalizada.obtenerAnio() << ":\n";
            ...
            entradaValida = true; // Exit the loop if input is valid
        } catch (const char* e) {
            std::cerr << "Error: " << e << "\n";
            std::cout << "Presione cualquier tecla para reintentar...";
            _getch();
