...
bool FechaHora::esFechaValida() const {
    // Obtener el año actual para establecer el límite superior
    FechaHora ahora;
    ...
    if (anio < 1900 || anio > anioActual) return false; // Ajustar rango a 1900-año actual
...
bool FechaHora::esIgualDia(const FechaHora& otra) const {
    return anio == otra.anio && mes == otra.mes && dia == otra.dia;
}

FechaHora FechaHora::normalizarDia() const {
    return FechaHora(dia, mes, anio, 0, 0, 0);
}